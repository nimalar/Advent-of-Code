...
fn main() 
{
	...
	'outer: while i < 50000000000
	{
		...
		for j in 0..pots.len()
		{
			if pots[j] == '#'
			{
				sum += j as i64 - zero_index as i64;
			}
		}
		let difference = sum - previous_sum;
		if difference == previous_difference && previous_sum != 0
		{
			println!("{}", sum + (50000000000 - i) * difference);
			break 'outer;
		}
		previous_sum = sum;
